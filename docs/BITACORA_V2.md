...
Se probó un lote nuevo de facturas colombianas reales dirigido a los tres casos pendientes de la sección 5.21 (IVA como renglón separado, fechas DD/MM/AAAA, NIT), con resultados desiguales entre los tres. El de IVA quedó verificado de punta a punta con una factura electrónica real que lo trae itemizado por renglón (Baterías Colombia SAS), reconciliando exacto contra el total; el caso de un vendedor no responsable de IVA también se confirmó correcto (impuesto guardado como `NULL`, no como cero). El de fecha DD/MM/AAAA funcionó para el mismo caso electrónico, pero reveló un caso límite nuevo, sin resolver: en una factura física con la fecha escrita a mano en tres casillas separadas (DIA/MES/AÑO), Textract no devolvió ningún campo `INVOICE_RECEIPT_DATE` en absoluto -- no es un error de interpretación del texto, es que el campo nunca llegó a detectarse. El de NIT resultó ser el más profundo de los tres: Textract sí lo detecta, pero como campo genérico `OTHER` sin distinguirlo de otros datos sueltos de la factura (incluyendo el NIT de otras partes, como el cliente o el proveedor del software de facturación) -- ver secciones 5.33 y 5.34, con una heurística ya implementada y verificada en producción, y una limitación conocida y aceptada explícitamente.

## 7. Próximos pasos

- Investigar el caso de fecha no detectada en facturas físicas con el formato de casillas separadas (DIA/MES/AÑO) -- descubierto al probar el NIT en la sección 5.34, sin investigar ni documentar todavía como limitación propia (a diferencia de las dos ya documentadas en `LIMITACIONES_CONOCIDAS.md`).
